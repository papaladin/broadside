...
(() => {
  ...
  const { T, panelStyle, Bar, Pill, Btn, StatBlock, SectionTitle, LogList, EmptyState, TutorialPopup, BackButton, Tooltip,
    ...
    // IconSwords, IconCannon, IconTarget, IconGrapple, IconWind, IconSkull,
    // IconRhum, IconSugar, IconTimber, IconCloth, IconSpice, IconSilk, IconCoffee, IconCocoa,
    // IconSpear, IconTobacco, IconGoblet, IconPerson
  } = window.UI;
  const { FactionPill, RepPill, ShipSprite } = window.UI;
  const { shouldShowTutorial, markTutorialSeen } = window.L;

/*   // ── Goods icon mapping (same as market screen) ──────────────────
  const GOOD_ICONS = {
    food: IconFood,
    water: IconWater,
    rum: IconRhum,
  ...
  const getGoodIcon = (good) => {
    ...
    return React.createElement(IconComponent, { size: 14, color: T.textDim, style: { marginRight: 6 } });
  }; */
...
//         <div style={{ textAlign: "center", color: T.redBr, fontSize: T.heading2FontSize, fontWeight: "bold", letterSpacing: "0.1em" }}>
...
//       {bs.phase === "victory" && (
//         <><IconAnchor size={24} color={T.greenBr} /> VICTORY!</>
//       )}
//       {bs.phase === "fled" && (
//         <><IconWind size={24} color={T.gold} /> ESCAPED</>
//       )}
//       {bs.phase === "defeat" && (
//         <><IconSkull size={24} color={T.redBr} /> DEFEATED</>
//       )}
//     </div>

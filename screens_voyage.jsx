...
function MapScreen({ state, dispatch }) {
  ...
  return (
    <div style={{
      ...
      {/* Map container with simple border (no hand‑drawn panel) */}
      <div style={{
              ...
              {Object.entries(PORTS).filter(([key]) => state.discoveredPorts?.includes(key)).map(([key, p]) => {
                ...
                return (
                  <g key={key} onClick={() => reachable && dispatch({ type: A.SAIL_TO, port: key })} onMouseEnter={() => setHov(key)} onMouseLeave={() => setHov(null)} style={{ cursor: reachable ? "pointer" : "default" }}>
                    ...
                    {/* ── TRADE PROFILE (Task 9) ────────────────────── */}
                    {isHov && (() => {
                      const profile = L.getPortTradeProfile(key);
                      ...
                      return (
                        <>
                          {tooltipDeals.length > 0 && (
                            <text x={p.x} y={p.y + 58} textAnchor="middle" fontSize="8" fill={T.greenBr} fontFamily={T.font}>
                              ✦ {tooltipDeals.map(g => D.RESOURCES[g]?.name).join(" · ")}
                            </text>
                          )}
                          {tooltipDemand.length > 0 && (
                            <text x={p.x} y={p.y + 68} textAnchor="middle" fontSize="8" fill={T.gold} fontFamily={T.font}>
  ...
  function SailingScreen({ state, dispatch }) {
    ...
    return (
      <div style={{
        ...
        {/* Voyage map with simple border (no hand‑drawn panel) */}
        <div style={{

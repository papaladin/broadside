...
(() => {
  ...
  const { T, panelStyle, Btn, Tooltip, NarrativePanel } = window.UI;
  const A = window.E.A;
  const L = window.L;
    ...
    const renderMenu = () => (
      <>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
          <Tooltip text="Return to the game">
            <Btn v="gold" style={{ width: "100%" }} onClick={closeAll}>Resume</Btn>
          </Tooltip>

          <Tooltip text="Record your progress against the perils of the sea.">
            <Btn style={{ width: "100%" }} onClick={() => { dispatch({ type: A.SAVE_GAME }); closeAll(); }}>Save Game</Btn>
          </Tooltip>

          {L.hasSave() && (
            <Tooltip text="Return to a previous record of your journey.">
              <Btn style={{ width: "100%" }} onClick={() => { dispatch({ type: A.LOAD_GAME }); closeAll(); }}>Load Game</Btn>
            </Tooltip>
          )}

          <Tooltip text="You will lose any unsaved progress.">
            <Btn v="ghost" style={{ width: "100%" }} onClick={() => pushPanel("newGame")}>Start New Game</Btn>
          </Tooltip>

          <Tooltip text="Save your adventure to a file for safekeeping.">
            <Btn style={{ width: "100%" }} onClick={() => { dispatch({ type: A.EXPORT_SAVE }); closeAll(); }}>Export Save</Btn>
          </Tooltip>

          <Tooltip text="Load an adventure from a file.">
            <Btn style={{ width: "100%" }} onClick={() => importRef.current?.click()}>Import Save</Btn>
          </Tooltip>
          <input type="file" accept=".broadside" ref={importRef} style={{ display: "none" }} onChange={handleImport} />
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, color: T.textDim, fontSize: T.captionFontSize }}>
        ...
        <div style={{ borderTop: `1px solid ${T.borderFaint}`, paddingTop: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <Btn v="ghost" style={{ width: "100%" }} onClick={() => window.open("handbook.html", "_blank")}>Captain's Handbook</Btn>
            <Btn v="ghost" style={{ width: "100%" }} onClick={() => pushPanel("changelog")}>Changelog</Btn>
            <Btn v="ghost" style={{ width: "100%" }} onClick={() => pushPanel("feedback")}>Send Feedback</Btn>
